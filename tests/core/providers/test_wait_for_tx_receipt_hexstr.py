...
def _send_dummy_tx(w3: Web3):
    ...
    return tx_hash  # HexBytes

def test_wait_for_tx_receipt_accepts_hex_string_success():
    w3 = Web3(Web3.EthereumTesterProvider())
    tx_hash = _send_dummy_tx(w3)          # HexBytes
    tx_hash_hexstr = tx_hash.hex()        # '0x...'

    # 文字列（hex str）でも receipt を取得できること
...
def test_wait_for_tx_receipt_accepts_hex_string_timeout():
    ...
    # 極端に短いタイムアウトで TimeExhausted が出るが、TypeError にはならないこと
